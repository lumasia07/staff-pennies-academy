...
Route::get('/dashboard', function () {
    ...
    $totalPassed = $completedTests->where('score', '>=', 80)->count();
...
// PDF route removed from public access — now under auth middleware below
...
Route::middleware('auth')->group(function () {
    ...
    Route::get('/switch-location/{location}', function ($location) {
        $user = \App\Models\User::whereRaw('LOWER(name) = ?', [strtolower($location)])->first();
        if ($user) {
            Auth::login($user);
            return redirect()->route('dashboard')->with('success', "Switched to $location profile.");
        }
        return redirect()->back()->with('error', "Location profile not found.");
